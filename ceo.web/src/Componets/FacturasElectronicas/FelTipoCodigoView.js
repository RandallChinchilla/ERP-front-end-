...
{ id: "codigoTipoCodigo", label: "Tipo Código", minWidth: 100 },
{ id: "descripcion", label: "Descripción", minWidth: 100 },
];

const FelTipoCodigoView = () => {
const { useState } = React;
const styles = useStyles();
const [columns, setColumns] = useState([
  {
    title: "Tipo Código",
    field: "codigoTipoCodigo",
  },
  { title: "Descripción", field: "descripcion" },
]);

const { Data, Error, setData } = useGetData("FelTipoCodigo");
console.log(Data);
...
const addState = (rowAdd) => {
  ...
  console.log(rowAdd);
  postAction(
    "FelTipocodigo/PostFelTipoCodigo",
    rowAdd,
    userLoggedToken
  ).then((response) => {
...
const updateState = (rowUpdate) => {
  const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
  putAction(
    "FelTipocodigo/PutFelTipoCodigo",
    rowUpdate,
    userLoggedToken
  ).then((response) => {
    if (response.status === 200) {
...
const deleteState = (rowDelete) => {
  ...
  deleteAction(
    "FelTipocodigo/DeleteFelTipoCodigo",
    rowDelete,
    userLoggedToken
  ).then((response) => {
    if (response.status === 200) {
...
export default FelTipoCodigoView;