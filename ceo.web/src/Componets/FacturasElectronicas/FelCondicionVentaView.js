...
const FelCondicionVentaView = () => {
  ...
  const [columns, setColumns] = useState([
    {
      title: "Condición Venta",
      field: "codigoCondicionVenta",
    },
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "FelCondicionVentum/PostFelCondicionVenta",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "FelCondicionVentum/DeleteFelCondicionVenta",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  return (
    <div>
      <MaterialTable
        title=" Catálogo Condición Venta"
        columns={columns}
        data={Data}
        editable={{
          onRowAdd: (newData) =>
