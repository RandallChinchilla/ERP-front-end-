/* import {
  ...
  const columns = [
    ...
    { id: "codigotipoimpuesto", label: "Tipo Impuesto", minWidth: 100 },
    { id: "descripcion", label: "Descripción", minWidth: 100 },
  ];
  
  
  const FelTipoImpuestoView = () => {
              ...
              <TableBody>
                {Data.map((row) => (
                  <TableRow key={row.codigoMarca}>
                    <TableCell>
                    <NavLink tag={Link} to="/Dashboard/FelTipoImpuesto">
                      <Edit
                    ...
                    <TableCell>{row.codigoTipoImpuesto}</TableCell>
  ...
  export default FelTipoImpuestoView; */
...
const columns = [
  { id: "acciones", label: "Acciones", minWidth: 125 },
  { id: "codigoTipoImpuesto", label: "Código Tipo Impuesto", minWidth: 100 },
  { id: "descripcion", label: "Descripción", minWidth: 100 },
  { id: "codigoTarifa", label: "Código Tarifa", minWidth: 100 },
...
const FelTipoImpuestoView = () => {
  ...
  const [columns, setColumns] = useState([
    {
      title: "Código Tipo Impuesto",
      field: "codigoTipoImpuesto",
    },
    { title: "Descripción", field: "descripcion" },
    { title: "Código Tarifa", field: "codigoTarifa" },
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "FelTipoimpuesto/PostFelTipoImpuesto",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "FelTipoimpuesto/PutFelTipoImpuesto",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "FelTipoimpuesto/DeleteFelTipoImpuesto",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
