...
const columns = [
  ...
  { id: "codigoTipoServicio", label: "Código Tipo Servicio", minWidth: 100 },
  { id: "descripcion", label: "Descripción", minWidth: 100 }
];

const CxpTipoServicio = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Tipo Servicio", field: "CodigoTipoServicio" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("CxpTipoServicio/GetCxpTipoServicios");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "CxpTipoServicio/PostCxpTipoServicio",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "CxpTipoServicio/PutCxpTipoServicio",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "CxpTipoServicio/DeleteCxpTipoServicio",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
...
export default CxpTipoServicio;