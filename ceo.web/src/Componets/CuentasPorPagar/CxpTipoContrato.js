...
const columns = [
  ...
  { id: "codigoTipoContrato", label: "Código Tipo Contrato", minWidth: 100 },
  { id: "descripcion", label: "Descripción", minWidth: 100 }
];

const CxpTipoContrato = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Tipo Contrato", field: "CodigoTipoContrato" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("CxpTipoContrato/GetCxpTipoContratos");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "CxpTipoContrato/PostCxpTipoContrato",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "CxpTipoContrato/PutCxpTipoContrato",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "CxpTipoContrato/DeleteCxpTipoContrato",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
...
export default CxpTipoContrato;