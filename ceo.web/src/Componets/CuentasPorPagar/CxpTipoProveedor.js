...
const columns = [
  ...
  { id: "codigoTipoProveedor", label: "Código Tipo Proveedor", minWidth: 100 },
  { id: "descripcion", label: "Descripción", minWidth: 100 }
];

const CxpTipoProveedor = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Tipo Proveedor", field: "CodigoTipoProveedor" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("CxpTipoProveedor/GetCxpTipoProveedores");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "CxpTipoProveedor/PostCxpTipoProveedor",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "CxpTipoProveedor/PutCxpTipoProveedor",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "CxpTipoProveedor/DeleteCxpTipoProveedor",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
...
export default CxpTipoProveedor;