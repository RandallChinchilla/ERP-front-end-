...
const columns = [
  ...
  { id: "CodigoEstadoCivil", label: "Código Estado Civil", minWidth: 100 },
...
const ParEstadoCivil = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Estado Civil", field: "CodigoEstadoCivil" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("ParEstadoCivil/GetParEstadosCiviles");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "ParEstadoCivil/PostParEstadoCivil",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "ParEstadoCivil/PutParEstadoCivil",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "ParEstadoCivil/DeleteParEstadoCivil",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
