...
const columns = [
  ...
  { id: "CodigoTipoFondo", label: "Código Tipo Fondo", minWidth: 100 },
...
const InvTipoFondo = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Concepto", field: "CodigoTipoFondo" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("InvTipofondo/GetInvTiposfondo");
  console.log(Data);
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "InvTipofondo/DeleteInvTipoFondo",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
