...
const columns = [
  ...
  { id: "CodigoTipoCustodia", label: "Código Tipo Custodia", minWidth: 100 },
  { id: "descripcion", label: "Descripción", minWidth: 100 }
];

const InvTipoCustodia = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Tipo Custodia", field: "CodigoTipoCustodia" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("InvTipocustodium/GetInvTiposCustodia");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "InvTipocustodium/PostInvTipoCustodia",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "InvTipocustodium/PutInvTipoCustodia",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "InvTipocustodium/DeleteInvTipoCustodia",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
...
export default InvTipoCustodia;