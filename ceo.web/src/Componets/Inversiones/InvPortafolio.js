...
const InvPortafolio = () => {
  ...
  const { Data, Error, setData } = useGetData("InvPortafolio/GetInvPortafolios");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "InvPortafolio/PostInvPortafolio",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "InvPortafolio/PutInvPortafolio",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "InvPortafolio/DeleteInvPortafolio",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
