...
const columns = [
  ...
  { id: "CodigoTipoPersona", label: "Código Tipo Persona", minWidth: 100 },
  { id: "Descripcion", label: "Descripción", minWidth: 100 },
];

const RegTipoPersona = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Tipo Persona", field: "CodigoTipoPersona" },
    { title: "Descripción", field: "Descripcion" },
  ]);

  const { Data, Error, setData } = useGetData("RegTipoPersona/GetRegTipoPersonas");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "RegTipoPersona/PostRegTipoPersona",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "RegTipoPersona/PutRegTipoPersona",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "RegTipoPersona/DeleteRegTipoPersona",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
