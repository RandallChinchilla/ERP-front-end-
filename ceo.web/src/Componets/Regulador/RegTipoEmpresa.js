...
const columns = [
  ...
  { id: "CodigoTipoEmpresa", label: "Código Tipo Empresa", minWidth: 100 },
  { id: "Descripcion", label: "Descripción", minWidth: 100 },
  { id: "Criterio", label: "Criterio", minWidth: 100 }
];

const RegTipoEmpresa = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Código Tipo Empresa", field: "CodigoTipoEmpresa" },
    { title: "Descripción", field: "Descripcion" },
    { title: "Criterio", field: "Criterio" },
  ]);

  const { Data, Error, setData } = useGetData("RegTipoEmpresa/GetRegTipoEmpresas");
  console.log(Data);
  ...
  const addState = (rowAdd) => {
    ...
    console.log(rowAdd);
    postAction(
      "RegTipoEmpresa/PostRegTipoEmpresa",
      rowAdd,
      userLoggedToken
    ).then((response) => {
  ...
  const updateState = (rowUpdate) => {
    const userLoggedToken = JSON.parse(localStorage.getItem("userLoggedToken"));
    putAction(
      "RegTipoEmpresa/PutRegTipoEmpresa",
      rowUpdate,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
  ...
  const deleteState = (rowDelete) => {
    ...
    deleteAction(
      "RegTipoEmpresa/DeleteRegTipoEmpresa",
      rowDelete,
      userLoggedToken
    ).then((response) => {
      if (response.status === 200) {
